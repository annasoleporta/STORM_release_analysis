...
rng(42,"twister"); % Fixar la llavor a un valor constant. A la línia 48, rand generarà sempre la mateixa seqüència 
% de nombres aleatoris, assegurant que el punt inicial sigui sempre el mateix en cada execució i, per tant, obteniu els mateixos resultats.

while numInitPts

    tempInd         = ceil( (numInitPts-1e-6)*rand);        %pick a random seed point /ceil=redondeo hacia + infinito, redondea a numero entero
    ...
    while 1     %loop untill convergence
        ...
        sqDistToAll = sum((repmat(myMean,1,numPts) - dataPts).^2);    %dist squared from mean to all points still active  Natalia: repmat(A,M,N) creates a large matrix B consisting of an M-by-N tiling of copies of A
