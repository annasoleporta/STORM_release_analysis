function ellipse_t = fit_ellipse(x,y,axis_handle,ScaleFactor,MaxParticleElongation,MaxParticleDiameter,isolated)
%
...
if (test<0)                                 %added by natalia: if test<0 then in the loop crashes because it doesn't have the parameter ellipse_t.valid
    ellipse_t.valid = 0;
end
...
if (nargin>2) & ~isempty( axis_handle ) & (test>0)
    ...
    ellipse_t.valid = 1; %ens quedem totes les el·lipses fiducials. ens és igual la forma
       % elongation 
       % ellipse_t.long_axis
    ...
    plot( new_horz_line(1,:),new_horz_line(2,:),color,'LineWidth', 1);
    plot( rotated_ellipse(1,:),rotated_ellipse(2,:),color,'LineWidth', 1);
